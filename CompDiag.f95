...
real(dp), parameter :: sym_eps = 2*10**(-103)
real(dp), parameter :: eps = 2*10**(-102)
real(dp), parameter :: dbl_eps = 2*10**(-52)
...
! Sq(c) = real(c*Conjg(c))
 ...
 do sweep = 1, 50
   ...
   do q = 2, n
     do p = 1, q - 1
       off = off + real(A(p,q)*conjg(A(p,q)))
     enddo
   enddo
